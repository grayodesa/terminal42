...
class Options_Machine {
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'slider':
					...
					$output .= '</ul>';
					$output .= '<a href="#" class="button slide_add_button">' . __( 'Add New Slide', 'coworker' ) . '</a></div>';
					
				break;
                
                //drag & drop sidebar generator
				case 'sidebargen':
					$output .= '<div class="slider"><ul id="'.$value['id'].'">';
					$sidebarsgen = $smof_data[$value['id']];
					$count = count($sidebarsgen);
					if ($count < 2) {
						$oldorder = 1;
						$order = 1;
						$output .= Options_Machine::optionsframework_sidebargen_function($value['id'],$value['std'],$oldorder,$order);
					} else {
						$i = 0;
						foreach ($sidebarsgen as $sidebargen) {
							$oldorder = $sidebargen['order'];
							$i++;
							$order = $i;
							$output .= Options_Machine::optionsframework_sidebargen_function($value['id'],$value['std'],$oldorder,$order);
						}
					...
					$output .= '</ul>';
					$output .= '<a href="#" class="button sidebar_add_button">' . __( 'Add New Sidebar', 'coworker' ) . '</a></div>';
					
				break;
                
                //drag & drop custom icons
				case 'customicons':
					$output .= '<div class="slider"><ul id="'.$value['id'].'">';
					$customicon_values = $smof_data[$value['id']];
					$count = count($customicon_values);
					if ($count < 2) {
						$oldorder = 1;
						$order = 1;
						$output .= Options_Machine::optionsframework_customicons_function($value['id'],$value['std'],$oldorder,$order);
					} else {
						$i = 0;
						foreach ($customicon_values as $customicon_value) {
							$oldorder = $customicon_value['order'];
							$i++;
							$order = $i;
							$output .= Options_Machine::optionsframework_customicons_function($value['id'],$value['std'],$oldorder,$order);
						}
					...
					$output .= '</ul>';
					$output .= '<a href="#" class="button customicon_add_button">' . __( 'Add New Icon', 'coworker' ) . '</a></div>';
				...
				case 'backup':
					...
					$output .= '<a href="#" id="of_backup_button" class="button" title="Backup Options">' . __( 'Backup Options', 'coworker' ) . '</a>';
					$output .= '<a href="#" id="of_restore_button" class="button" title="Restore Options">' . __( 'Restore Options', 'coworker' ) . '</a>';
					$output .= '</div>';
				...
				case 'transfer':
					...
					$output .= '<a href="#" id="of_import_button" class="button" title="Restore Options">' . __( 'Import Options', 'coworker' ) . '</a>';
	...
	public static function optionsframework_slider_function($id,$std,$oldorder,$order){
		...
		if (!empty($val['title'])) {
			...
			$slider .= '<li><div class="slide_header"><strong>' . __( 'Slide', 'coworker' ) . ' '.$order.'</strong>';
		}
		
		$slider .= '<input type="hidden" class="slide of-input order" name="'. $id .'['.$order.'][order]" id="'. $id.'_'.$order .'_slide_order" value="'.$order.'" />';
	
		$slider .= '<a class="slide_edit_button" href="#">' . __( 'Edit', 'coworker' ) . '</a></div>';
		
		$slider .= '<div class="slide_body">';
		
		$slider .= '<label>' . __( 'Title', 'coworker' ) . '</label>';
		...
		$slider .= '<label>' . __( 'Image URL', 'coworker' ) . '</label>';
		$slider .= '<input class="upload slide of-input" name="'. $id .'['.$order.'][url]" id="'. $id .'_'.$order .'_slide_url" value="'. $val['url'] .'" />';
		
		$slider .= '<div class="upload_button_div"><span class="button media_upload_button" id="'.$id.'_'.$order .'">' . __( 'Upload', 'coworker' ) . '</span>';
		
		if(!empty($val['url'])) {$hide = '';} else { $hide = 'hide';}
		$slider .= '<span class="button remove-image '. $hide.'" id="reset_'. $id .'_'.$order .'" title="' . $id . '_'.$order .'">' . __( 'Remove', 'coworker' ) . '</span>';
		$slider .='</div>' . "\n";
		...
		$slider .= '</div>';	
		$slider .= '<label>' . __( 'Link URL (optional)', 'coworker' ) . '</label>';
		$slider .= '<input class="slide of-input" name="'. $id .'['.$order.'][link]" id="'. $id .'_'.$order .'_slide_link" value="'. $val['link'] .'" />';
		
		$slider .= '<label>' . __( 'Description (optional)', 'coworker' ) . '</label>';
		$slider .= '<textarea class="slide of-input" name="'. $id .'['.$order.'][description]" id="'. $id .'_'.$order .'_slide_description" cols="8" rows="8">'.stripslashes($val['description']).'</textarea>';
	
		$slider .= '<a class="slide_delete_button" href="#">' . __( 'Delete', 'coworker' ) . '</a>';
	 ...
	 * @return string
	 */
	public static function optionsframework_sidebargen_function($id,$std,$oldorder,$order){
		...
		if (!empty($val['title'])) {
			...
			$slider .= '<li><div class="slide_header"><strong>' . __( 'Sidebar', 'coworker' ) . ' '.$order.'</strong>';
		}
		...
		$slider .= '<label>' . __( 'Sidebar Name', 'coworker' ) . '</label>';
		$slider .= '<input class="slide of-input of-slider-title" name="'. $id .'['.$order.'][title]" id="'. $id .'_'.$order .'_slide_title" value="'. stripslashes($val['title']) .'" />';
	 ...
	 * @return string
	 */
	public static function optionsframework_customicons_function($id,$std,$oldorder,$order){
		...
		if (!empty($val['title'])) {
			...
			$slider .= '<li><div class="slide_header"><strong>' . __( 'Icon', 'coworker' ) . ' '.$order.'</strong>';
		}
		...
		$slider .= '<label>' . __( 'Icon Name', 'coworker' ) . '</label>';
		$slider .= '<input class="slide of-input of-slider-title" name="'. $id .'['.$order.'][title]" id="'. $id .'_'.$order .'_slide_title" value="'. stripslashes($val['title']) .'" />';
		
		$slider .= '<label>' . __( 'Icon Image URL', 'coworker' ) . '</label>';
		...
		$slider .= '</div>';	
		$slider .= '<label>' . __( 'Icon Link URL', 'coworker' ) . '</label>';
		$slider .= '<input class="slide of-input" name="'. $id .'['.$order.'][link]" id="'. $id .'_'.$order .'_slide_link" value="'. $val['link'] .'" />';
