...
function BWWC__getreceivedbyaddress_info ($address_request_array, $bwwc_settings=false)
{
  ...
  if (!is_numeric($funds_received))
  {
   ...
   $funds_received = BWWC__file_get_contents ('https://blockchain.info/q/getreceivedbyaddress/' . $btc_address . $confirmations_url_part_bci, true, $api_timeout);
   ...
   if (!is_numeric($funds_received))
   {
    ...
    $funds_received = BWWC__file_get_contents ('https://blockexplorer.com/api/addr/' . $btc_address . '/totalReceived', true, $api_timeout);
