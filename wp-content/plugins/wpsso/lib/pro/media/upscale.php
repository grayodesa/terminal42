...
if ( ! class_exists( 'WpssoProMediaUpscale' ) ) {
	...
	class WpssoProMediaUpscale {
		 ...
		 * and check for our own image sizes, and use in status and warning notices.
		 */
		public function upscale_image_resize_dimensions( $ret, $orig_w, $orig_h, $dst_w, $dst_h, $crop ) {
			 ...
			 */
			$img_info = WpssoMedia::get_image_src_info();
			 ...
			 */
			if ( ( empty( $img_info['size_name'] ) || strpos( $img_info['size_name'], $this->p->cf['lca'].'-' ) !== 0 ) &&
				! apply_filters( $this->p->cf['lca'].'_image_upscale_all', 
					SucomUtil::get_const( 'WPSSO_IMAGE_UPSCALE_ALL' ) ) ) {

				if ( $this->p->debug->enabled )
					$this->p->debug->log( 'upscale skipped - not '.$this->p->cf['lca'].
						' size name and upscale all is false' );
				return $ret;
			}
			...
			else return array( 0, 0, (int) $src_x, (int) $src_y,
