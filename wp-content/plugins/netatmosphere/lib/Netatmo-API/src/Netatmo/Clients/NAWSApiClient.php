...
class NAWSApiClient extends NAApiClient
{
   ...
   */
   ...
   {
        /*include_once(NAS_PLUGIN_ROOT . '/inc/class-nas-plugin.php');
        \NAS_Plugin::debugFile($device_id, __FILE__, __LINE__, __FUNCTION__ . " - device_id");

        $tmp = $this->api('getstationsdata', 'GET', array($device_id, true));
        \NAS_Plugin::debugFile($tmp, __FILE__, __LINE__, __FUNCTION__ . " - result 4 get_favorites = true");

        $tmp = $this->api('getstationsdata', 'GET', array($device_id, false));
        \NAS_Plugin::debugFile($tmp, __FILE__, __LINE__, __FUNCTION__ . " - result 4 get_favorites = false");
