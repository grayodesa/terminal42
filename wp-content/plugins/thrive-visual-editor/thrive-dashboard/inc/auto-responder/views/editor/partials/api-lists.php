			...
			<?php else : ?>
				<div class="tve_lightbox_select_holder tve_lightbox_input_inline tve_lightbox_select_inline">
					<select id="thrive-api-list-select"<?php echo ( empty( $lists ) ) ? ' disabled' : 'class="tve_change"
				        data-ctrl="function:auto_responder.api.api_get_groups" data-api="' . $api_key .  '"' ?>  >
						<?php if ( empty( $lists ) ) : ?>
							<option value=""><?php echo __( 'No list available', TVE_DASH_TRANSLATE_DOMAIN ) ?></option>
						<?php endif ?>
						<?php foreach ( $lists as $list ) : ?>
							<option data-group="<?php echo isset($list['group_count']) ? $list['group_count'] : 0; ?>" value="<?php echo $list['id'] ?>"<?php echo ! empty( $selected_list ) && $selected_list == $list['id'] ? ' selected="selected"' : '' ?>><?php echo $list['name'] ?></option>
						<?php endforeach ?>
					</select>
				</div>
				&nbsp;&nbsp;&nbsp;
