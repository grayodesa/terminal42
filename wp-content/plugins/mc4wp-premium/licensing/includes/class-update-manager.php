...
if( ! class_exists( 'DVK_Update_Manager', false ) ) {
	...
	class DVK_Update_Manager {
		...
		private function call_remote_api() {
			...
			// TODO: Take lifetime licenses into account here.. (if we ever will support that)
			...
			// store response
			set_site_transient( $this->product->slug . '-update-data', $response, 10800 );
		...
		private function get_cached_remote_data() {
			...
			$data = get_site_transient( $this->product->slug . '-update-data' );
