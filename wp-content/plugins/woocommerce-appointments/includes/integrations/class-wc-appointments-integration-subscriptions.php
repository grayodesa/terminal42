 ...
 * @since 1.0.0
 */
class WC_Appointments_Integration_Subscriptions {
	 ...
	 * @return WC_Appointments_User_Membership|null
	 */
	...
	public function handle_subscription_status_change_1_5( $user_id, $subscription_key ) {
		...
		switch ( current_filter() ) {
			...
			case 'subscription_trashed':
				$note = __( 'Membership cancelled because subscription was trashed.', 'woocommerce-appointments' );
				break;

			case 'subscription_deleted':
				$note = __( 'Membership cancelled because subscription was deleted.', 'woocommerce-appointments' );
				break;
		}
	 ...
	 * @param WC_Appointments_User_Membership $user_membership
	 * @param string $status
		...
		switch ( $status ) {
			...
			case 'active':
				...
				if ( $trial_end && $trial_end > current_time( 'timestamp', true ) ) {
					...
					if ( ! $note ) {
						$note = __( 'Membership free trial activated because subscription was re-activated.', 'woocommerce-appointments' );
					}
					...
					if ( ! $note ) {
						$note = __( 'Membership activated because subscription was re-activated.', 'woocommerce-appointments' );
					}
			...
			case 'on-hold':
				...
				if ( ! $note ) {
					$note = __( 'Membership paused because subscription was put on-hold.', 'woocommerce-appointments' );
				}
			...
			case 'expired':
				...
				if ( ! $note ) {
					$note = __( 'Membership expired because subscription expired.', 'woocommerce-appointments' );
				}
			...
			case 'pending-cancel':
				...
				if ( ! $note ) {
					$note = __( 'Membership marked as pending cancellation because subscription is pending cancellation.', 'woocommerce-appointments' );
				}
			...
			case 'cancelled':
				...
				if ( ! $note ) {
					$note = __( 'Membership cancelled because subscription was cancelled.', 'woocommerce-appointments' );
				}
	 ...
	 * @param WC_Appointments_Membership_Plan_rule $rule Related rule
	 * @param WC_Appointments_User_Membership $user_membership
	 * @return string Modified from_time, as timestamp
	 */
	public function adjust_post_access_from_time( $from_time, WC_Appointments_Membership_Plan_Rule $rule, WC_Appointments_User_Membership $user_membership ) {
	 ...
	 */
	public function renew_membership_url( $url, WC_Appointments_User_Membership $user_membership ) {
	 ...
	 * @since 1.0.0
	 * @param bool $renew
	 * @param WC_Appointments_Membership_Plan $plan
	 * @param array $args
	 * @return bool
	 */
	 ...
	 */
	public function adjust_access_granting_product_id( $product_id, $access_granting_product_ids, WC_Appointments_Membership_Plan $plan ) {
	 ...
	 * @param array $args
	 */
	public function save_subscription_data( WC_Appointments_Membership_Plan $plan, $args ) {
	...
	public function add_free_trial_status( $statuses ) {
		...
		$statuses = SV_WC_Helper::array_insert_after( $statuses, 'wcm-active', array(
			'wcm-free_trial' => array(
				'label'       => _x( 'Free Trial', 'Membership Status', 'woocommerce-appointments' ),
				'label_count' => _n_noop( 'Free Trial <span class="count">(%s)</span>', 'Free Trial <span class="count">(%s)</span>', 'woocommerce-appointments' ),
			)
		) );
	 ...
	 * @param WC_Appointments_User_Membership $user_membership
	 */
	public function output_subscription_details( WC_Appointments_User_Membership $user_membership ) {
		...
		$subscription_expires = $this->is_subscriptions_gte_2_0()
									...
									: $subscription['expiry_date'] ? date_i18n( wc_date_format(), strtotime( $subscription['expiry_date'] ) ) : __( 'Subscription not yet ended', 'woocommerce-appointments' );
		?>
			<table>
				<tr>
					<td><?php _e( 'Subscription:', 'woocommerce-appointments' ); ?></td>
					<td><a href="<?php echo $subscription_link ?>"><?php echo $subscription_link_text ?></a></td>
				</tr>
				<tr>
					<td><?php _e( 'Next Bill On:', 'woocommerce-appointments' ); ?></td>
					<td><?php echo $next_payment ? date_i18n( wc_date_format(), $next_payment ) : __( 'N/A', 'woocommerce-appointments' ); ?></td>
				</tr>
			</table>
		<?php
	...
	public function output_subscription_column_headers() {
		?><th class="membership-next-bill-on"><span class="nobr"><?php _e( 'Next Bill On', 'woocommerce-appointments' ); ?></span></th><?php
	}
	 ...
	 * @param WC_Appointments_User_Membership $user_membership
	 */
	public function output_subscription_columns( WC_Appointments_User_Membership $user_membership ) {
		...
		?>
		<td class="membership-membership-next-bill-on" data-title="<?php esc_attr_e( 'Next Bill On', 'woocommerce-appointments' ); ?>">
			<?php if ( $subscription && ! empty( $next_payment ) ) : ?>
				<?php echo date_i18n( get_option( 'date_format' ), $next_payment ) ?>
			<?php else : ?>
				<?php _e( 'N/A', 'woocommerce-appointments' ); ?>
			<?php endif; ?>
		</td>
		<?php
	}
	...
	public function output_subscription_options() {
		...
		if ( $this->plan_grants_access_while_subscription_active( $post->ID ) ) {
			...
			<p class="subscription-access-notice <?php if ( ! $has_subscription ) : ?>hide<?php endif; ?> js-show-if-has-subscription">
				<span class="description"><?php _e( 'Membership will be active while the purchased subscription is active.', 'woocommerce-appointments' ); ?></span>
				<img class="help_tip" data-tip='<?php _e( 'If membership access is granted via the purchase of a subscription, then membership length will be automatically equal to the length of the subscription, regardless of the membership length setting above.', 'woocommerce-appointments' ) ?>' src="<?php echo WC()->plugin_url(); ?>/assets/images/help.png" height="16" width="16" />
			</p>
	...
	public function output_exclude_trial_option( $rule, $index ) {
		...
		<span class="rule-control-group rule-control-group-access-schedule-trial <?php if ( ! $has_subscription ) : ?>hide<?php endif; ?> js-show-if-has-subscription">
			...
			<label for="_<?php echo $type; ?>_rules_<?php echo $index; ?>_access_schedule_exclude_trial" class="label-checkbox">
				<?php _e( 'Start after trial', 'woocommerce-appointments' ); ?>
			</label>
			...
			if ( $subscription_status != $user_membership->get_status() ) {
				...
				if ( 'paused' == $user_membership->get_status() ) {
					...
					if ( ! $trial_end || current_time( 'timestamp' ) >= strtotime( $trial_end ) ) {
						$user_membership->activate_membership( __( 'Membership activated because WooCommerce Subscriptions was activated.', 'woocommerce-appointments' ) );

					// Otherwise, put it on free trial
					} else {
						$user_membership->update_status( 'free_trial', __( 'Membership free trial activated because WooCommerce Subscriptions was activated.', 'woocommerce-appointments' ) );
					}
	...
	public function pause_free_trial_subscription_memberships() {
		...
		foreach ( $posts as $post ) {
			$user_membership = wc_memberships_get_user_membership( $post );
			$user_membership->pause_membership( __( 'Membership paused because WooCommerce Subscriptions was deactivated.', 'woocommerce-appointments' ) );
		}
	}
...
$GLOBALS['wc_appointments_integration_subscriptions'] = new WC_Appointments_Integration_Subscriptions();
