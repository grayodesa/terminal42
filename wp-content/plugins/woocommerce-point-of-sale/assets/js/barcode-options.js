jQuery(document).ready(function($) {
	var wc_pos_barcode_options = {
		...
		print_barcodes: function () {
			if( !$( '#edit_wc_pos_barcode #order_line_items tr.item' ).length ){ return false; }
			...
			switch(type){
				case 'continuous_feed':
					...
					$('#printable_barcode').removeClass('a4').removeClass('letter');
				break;
				case 'a4':
					$('#printable_barcode').addClass('a4');
					$('#printable_barcode').removeClass('continuous_feed').removeClass('letter');
				break;
				case 'letter':
					$('#printable_barcode').addClass('letter');
					$('#printable_barcode').removeClass('continuous_feed').removeClass('a4');
				break;
			}
			window.print(); 
		},
		...
		fields_to_print: function () {
			...
			$('#order_line_items .item').each(function(index, el) {
				...
				if( !field_barcode ){
					...
					if( $(el).find('.sku_text').length && field_sku){
						var sku = $(el).find('.sku_text').text();
						$barcode_text.append(sku).append('<br />');						
					}
				}else{
					$(el).find('.barcode_border img').show();
				}
				if( !field_sku ){
					$(el).find('.barcode_border img').attr('src', barcode_url + '&font_size=0');
				}else{
					$(el).find('.barcode_border img').attr('src', barcode_url + '&font_size=14');
				}
				if( field_name ){
