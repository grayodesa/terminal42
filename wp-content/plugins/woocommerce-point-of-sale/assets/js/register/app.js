...
jQuery(document).ready(function($) {     
	...
	APP       = window.POS_APP = {
        ...
        sync_data : function(block){
            if( APP.processing_payment === true ) return;
                    ...
                    $.each(productsData.products, function(index, product) {
                        ...
                        if( product.variations.length > 0 ){
                            $.each(product.variations, function(i, variation) {
                                APP.db.put('variations', {id: variation.id, prod_id: product.id, title: product.title, sku: variation.sku });
                            });
                        }
                    });
...
*/
				// Custom Handler
				} else if (  wp.hooks.hasAction('wc_pos_add_to_cart_handler_' + add_to_cart_handler)  ){
					wp.hooks.doAction( 'wc_pos_add_to_cart_handler_' + add_to_cart_handler, product_id, adding_to_cart, quantity, variation_id, variation, cart_item_data );
        ...
        add_to_cart_handler_simple : function (product_id, adding_to_cart, quantity, variation_id, variation, cart_item_data) {
			...
			window.openwin = false;
			var passed_validation 	=  wp.hooks.applyFilters( 'wc_pos_add_to_cart_validation', true, product_id, quantity );
			var cart_item_key       = '';
        ...
        add_to_cart_handler_variable : function (product_id, adding_to_cart, quantity, variation_id, selected_attr, cart_item_data) {
			...
			if( missing === true ){
				...
				window.openwin = false;
				var passed_validation 	=  wp.hooks.applyFilters( 'wc_pos_add_to_cart_validation', true, product_id, quantity, variation_id, variations );
				var cart_item_key       = '';
		...
		createOrder : function(paid){
			...
			if ( Object.size(CART.applied_coupons) > 0 ) {
				...
				$.each(CART.applied_coupons, function(index, coupon_code) {
					...
					if ( amount ) {
						cart.order.coupon_lines.push({'amount' : amount, 'code' : coupon_code});
					}
				});	
	    ...
	    ready: function () {           
                ...
                query: function (query) {
                    ...
                    APP.db.count('products').done(function(x) {
                        ...
                        APP.db.from('products').order('sku').list(x).done(function(records) {
                            ...
                            $.each(records, function(index, e) {
                                var sku   = e.sku;
                                var title = e.title;
                                    sku   = sku.toLowerCase();
                                    title = title.toLowerCase();

                                    if( title.indexOf(_term) >= 0 || ( e.sku != '' && sku.indexOf(_term) >= 0 ) ){
                                        
                                        var t = e.title;
                                        if( e.sku != ''){
                                            t = e.sku + ' - ' + e.title;
                                        }
                                        var val = JSON.stringify({id: e.id});
                                        result.push({ id: val, text: t });
                                    }
                                    if( e.variations.length > 0 ){
                                        $.each( e.variations, function(i, v) {
                                            if( v.sku != '' ){
                                                var sku   = v.sku;
                                                    sku   = sku.toLowerCase();
                                                
                                                if( sku.indexOf(_term) >= 0 ){
                                                    var t = v.sku + ' - ' + e.title;
                                                    var selected_attr = {};

                                                    $.each(v.attributes, function(j, a) {
                                                        if( a.option != '' ){
                                                    ...
                                                    var val = JSON.stringify({id: e.id, vid: v.id, variation: selected_attr});
                                                    result.push({ id: val, text: t });
                                                }
                                            }
                                        });
            ...
            $('#edit_wc_pos_registers').on('click', '.add_grid_tile', function(event) {
	            ...
	            if (product_id && product_id != '') {                
	                APP.addToCart(product_id, 1, variation_id);
	            }
            });
			...
			$('#modal-order_payment').on('click', 'input.go_payment', function(){
                ...
                if( ( $('#amount_pay_cod').val() == '' || amount_pay < total_amount ) && selected_pm == 'cod') {
            ...
            $('#order_items_list').on('change', '.product_price', function(e){ changeProductPrice($(this)); });
            $('#order_items_list').on('keyup', '.product_price', function(e){ changeProductPrice($(this)); });
