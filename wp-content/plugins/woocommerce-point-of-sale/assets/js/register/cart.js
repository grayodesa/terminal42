jQuery(document).ready(function($) { 
	window.POS_CART = {
		...
		init : function () {
			wp.hooks.addAction( 'wc_pos_add_to_cart', CART.calculate_totals, 20, 0);
			wp.hooks.addAction( 'wc_pos_applied_coupon', CART.calculate_totals, 20, 0);
		},
		/*-----------------------------------------------------------------------------------*/
		...
		addToCart : function (product_data, product_id, quantity, variation_id, variation, cart_item_data) {
			...
			try{
				...
				if(variation_data){
					if ( variation_data.in_stock === false && variation_data.backordered === false ) {
					...
					if ( product_data.in_stock === false && product_data.backordered === false ) {
						throw new Error( sprintf(pos_i18n[3], product_data.title ) );
					}
				...
				if ( cart_item_key ) {
					var new_quantity = quantity + CART.cart_contents[ cart_item_key ]['quantity'];
					...
					CART.cart_contents[ cart_item_key ] = _extend;
					
					var source   = $('#tmpl-cart-product-item').html();
					var template = Handlebars.compile(source);
					var context  = {cart_item_data: cart_item_data, cart_item_key: cart_item_key};
		...
		has_enough_stock : function(product_data, quantity){
			...
			var has_enough_stock = product_data.managing_stock === false || product_data.backordered === true || product_data.stock_quantity >= quantity ? true : false;
		...
		calculate_totals : function () {
			 ...
			 */
			 $.each(cart, function(cart_item_key, values) {
				...
				// Prices
				var line_price = parseFloat(_product.price) * values['quantity'];
			...
			$.each(cart, function(cart_item_key, values) {
				 ...
				 */
					...
					if ( item_tax_rates !== base_tax_rates  ) {
						...
						// Now we have a new item price (excluding TAX)
						var line_subtotal     = round( line_price - array_sum( taxes ), pos_wc.precision );
							taxes             = TAX.calc_tax( line_subtotal, item_tax_rates );
						var line_subtotal_tax = array_sum( taxes );

						var line_o_subtotal     = round( line_o_price - array_sum( o_taxes ), pos_wc.precision );
						...
						var discounted_price  = CART.get_discounted_price( values, adjusted_price, true );
							discounted_taxes  = TAX.calc_tax( discounted_price * values['quantity'], item_tax_rates, true );
						var line_tax          = array_sum( discounted_taxes );
						var line_total        = ( discounted_price * values['quantity'] ) - line_tax;
				...
				if(CART.prices_include_tax){
					if( line_subtotal != line_total || line_price != line_o_price){
						var line_total_row = '<del>' + accountingPOS(line_o_subtotal+line_o_subtotal_tax, 'formatMoney') + '</del><span class="amount">' + accountingPOS(line_total+line_tax, 'formatMoney') + '</span>';
					}else{
						var line_total_row = '<span class="amount">' + accountingPOS(line_total+line_tax, 'formatMoney') + '</span>';
					}
				}else{
					if( line_subtotal != line_total || line_price != line_o_price){
						var line_total_row = '<del>' + accountingPOS(line_o_subtotal, 'formatMoney') + '</del><span class="amount">' + accountingPOS(line_total, 'formatMoney') + '</span>';
					}else{
						var line_total_row = '<span class="amount">' + accountingPOS(line_total, 'formatMoney') + '</span>';
					}					
				}
			...
			if ( CART.needs_shipping() ) {
				...
				var template  = Handlebars.compile(source);
				var ship      = { title : CART.chosen_shipping_methods.title, price : accountingPOS(CART.chosen_shipping_methods.price, 'formatMoney') }
				...
				if( pos_cart.tax_total_display == 'itemized'){
					...
					$.each(CART.taxes, function(index, amount) {
						...
						if( typeof pos_wc.all_rates[index] != 'undefined' ){
							tax_data[index] = { 'label' : pos_wc.all_rates[index]['label'], 'amount': accountingPOS(amount, 'formatMoney') };							
						}
					});
				}else{
					var tax_data = {1:{ 'label' : pos_cart.tax_or_vat, 'amount': accountingPOS(CART.tax_total, 'formatMoney') }};
				}
