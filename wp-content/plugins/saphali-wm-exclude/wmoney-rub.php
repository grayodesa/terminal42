<?php
class wmoney_rub  extends WC_Payment_Gateway {
	// WebMoney
	private $wmpurse_rur;
	private $wpfailUrl;
	private $wpresultUrl;
	...
	public function __construct () {
		...
		$this->wmpurse_rur = get_option('wmpurse_rur');
		...
		if(empty($this->title))
		add_option('woocommerce_wmoney_title'  . $this->id, __('WebMoney (WMR)', 'loc-saphali-wm') );
	...
	function is_valid_for_use_cur() {
		if (in_array(get_option('woocommerce_currency'), array('BYR') )) {
			return false;
		}
		return true;
	}
		...
		if( isset($posted['LMI_PREREQUEST']) && $posted['LMI_PREREQUEST'] == 1){ # Prerequest
					...
					if ( !($order_r = $order->get_order($posted['LMI_PAYMENT_NO'])) ) {
						...
						if($posted['FIELD_2'] == 'USD') {
							...
							if($get_woocommerce_currency == 'UAH') {
								$order->order_total = $order->order_total/$curs_usd;
							...
							$_PURSE_wm = get_option('wmpurse_uah');
				...
				if ( !($order_r = $order->get_order($posted['LMI_PAYMENT_NO'])) ) {
					...
					&& $hash_check ) {  # checksum is correct, step 15
						...
						if ($this->debug=='yes') $this->log->add( $this->id, 'Заказ #' . $posted['LMI_PAYMENT_NO'] . ". Inconsistent parameters" . '# step 15' . print_r($posted, true) .  print_r($sha256 . '|' . $md5sum . '|' . $posted['LMI_HASH'] , true) );
...
<?php
		...
		public function process_admin_options () {
			if($_POST['woocommerce_wmoney_title'] . $this->id) {
				...
				update_option('wmpurse_rur',$_POST['wmpurse_rur']);
	...
	public function generate_form( $order_id ) {
		...
		if($get_woocommerce_currency == 'UAH') {
			$order->order_total = number_format($order->order_total/$curs_rur, 2, '.', '');
			$_PURSE_wm = "<input type='hidden' name='LMI_PAYEE_PURSE' value='{$this->wmpurse_rur}'>";
		}elseif($get_woocommerce_currency == 'RUR' || $get_woocommerce_currency == 'RUB') {
			$_PURSE_wm = "<input type='hidden' name='LMI_PAYEE_PURSE' value='{$this->wmpurse_rur}'>";
		}elseif($get_woocommerce_currency == 'USD') {
			$order->order_total = number_format($order->order_total*($curs_usd/$curs_rur), 2, '.', '');
