<?php
class wmoney_usd  extends WC_Payment_Gateway {
	// WebMoney
	private $wmpurse_usd;
	private $wpfailUrl;
	private $wpresultUrl;
	...
	public function __construct () {
		...
		$this->wmpurse_usd = get_option('wmpurse_usd');
		...
		if(empty($this->title))
		add_option('woocommerce_wmoney_title'  . $this->id, __('WebMoney (WMZ)', 'loc-saphali-wm') );
...
<?php
		...
		public function process_admin_options () {
			if($_POST['woocommerce_wmoney_title'] . $this->id) {
				...
				update_option('wmpurse_usd',$_POST['wmpurse_usd']);
	...
	public function generate_form( $order_id ) {
		...
		if($get_woocommerce_currency == 'UAH') {
			$order->order_total = number_format($order->order_total/$curs_usd, 2, '.', '');
			$_PURSE_wm = "<input type='hidden' name='LMI_PAYEE_PURSE' value='{$this->wmpurse_usd}'>";
		}elseif($get_woocommerce_currency == 'RUR' || $get_woocommerce_currency == 'RUB') {
			$order->order_total = number_format($order->order_total/($curs_usd/$curs_rur), 2, '.', '');
			$_PURSE_wm = "<input type='hidden' name='LMI_PAYEE_PURSE' value='{$this->wmpurse_usd}'>";
		}elseif($get_woocommerce_currency == 'USD') {
