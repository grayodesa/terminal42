...
/**
 * Class Tribe__Tickets_Plus__Commerce__WooCommerce__Orders_Table
 *
 * See documentation for WP_List_Table
 */
class Tribe__Tickets_Plus__Commerce__WooCommerce__Orders_Table extends WP_List_Table {
	public $event_id;
	...
	public function get_columns() {
		$columns = array(
			'order'      => esc_html__( 'Order', 'event-tickets' ),
			'purchased'  => esc_html__( 'Purchased', 'event-tickets' ),
			'ship_to'    => esc_html__( 'Ship to', 'event-tickets' ),
			'date'       => esc_html__( 'Date', 'event-tickets' ),
		);

		if ( self::event_fees( $this->event_id ) ) {
			$columns['subtotal'] = esc_html__( 'Subtotal', 'event-tickets' );
			$columns['site_fee'] = esc_html__( 'Site Fee', 'event-tickets' );
		}

		$columns['total'] = esc_html__( 'Total', 'event-tickets' );
	...
	public function column_order( $item ) {
		...
		$output = "{$order_number_link} " . esc_html__( 'by', 'event-tickets' ) . " {$customer_name}<br><a href=\"mailto:{$customer_email}\">{$customer_email}</a>";
